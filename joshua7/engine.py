...
def _filter_overrides(validator_name: str, overrides: dict[str, Any]) -> dict[str, Any]:
    """Strip security-sensitive keys from per-request config overrides.

    Two-layer defense: first restrict to the safe allowlist, then remove
    any validator-specific blocked keys.
    """
    safe = {k: v for k, v in overrides.items() if k in _SAFE_OVERRIDE_KEYS}
    blocked = _BLOCKED_OVERRIDE_KEYS.get(validator_name, frozenset())
    if not blocked:
        return safe
    filtered = {k: v for k, v in safe.items() if k not in blocked}
