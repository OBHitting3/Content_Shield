...
class TestAPI:
    ...
    def test_malicious_request_id_sanitized(self, client):
        """SECURITY: Malicious X-Request-ID headers must be sanitized."""
        resp = client.post(
            "/api/v1/validate",
            json={
                "text": "Sanitize test.",
    ...
    def test_injection_detected_in_response(self, client):
        """SECURITY: Prompt injection must be detected and flagged."""
        resp = client.post(
            "/api/v1/validate",
            json={
        ...
        data = resp.json()
        assert data["passed"] is False
        pi = next(r for r in data["results"] if r["validator_name"] == "prompt_injection")
        assert pi["passed"] is False
        assert len(pi["findings"]) > 0
