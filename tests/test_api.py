...
class TestValidation:
    ...
    def test_validate_all_default(self, client):
        resp = client.post(
            "/api/v1/validate",
            json={
                "text": (
                    "Our team is dedicated to quality and innovation "
...
class TestResponseMetadata:
    ...
    def test_response_contains_timestamp(self, client):
        resp = client.post(
            "/api/v1/validate",
            json={"text": "Timestamp check.", "validators": ["forbidden_phrases"]},
        )
        data = resp.json()
    ...
    def test_response_contains_version(self, client):
        resp = client.post(
            "/api/v1/validate",
            json={"text": "Version check.", "validators": ["forbidden_phrases"]},
        )
        data = resp.json()
        assert "version" in data
        assert data["version"] != ""
    ...
    def test_response_time_header(self, client):
        resp = client.post(
            "/api/v1/validate",
            json={"text": "Timing check.", "validators": ["forbidden_phrases"]},
        )
...
class TestRiskInResponse:
    def test_risk_present_in_json(self, client):
        resp = client.post(
            "/api/v1/validate",
            json={"text": "Test content for risk.", "validators": ["all"]},
        )
        data = resp.json()
        assert "risk" in data
