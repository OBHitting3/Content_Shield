...
config override sandboxing, PII redaction, prompt injection patterns, and
...
def rate_limited_client(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("J7_RATE_LIMIT_BURST", "0")
    app = create_app()
    return TestClient(app)
...
# ---------------------------------------------------------------------------
# Input sanitization (from sanitize.py)
# ---------------------------------------------------------------------------
...
class TestRequestBodyLimit:
    ...
    def test_oversized_body_rejected(self, small_body_client):
        large_body = '{"text": "' + "A" * 500 + '", "validators": ["forbidden_phrases"]}'
...
class TestConfigOverrideSandboxing:
    ...
    def test_pii_patterns_override_blocked_in_engine(self):
...
# ---------------------------------------------------------------------------
# PII Security (never leaked + credit card detection)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Injection metadata safety
# ---------------------------------------------------------------------------


class TestInjectionMetadataSafety:
    def test_matched_text_in_metadata_is_truncated(self, client):
        resp = client.post(
            "/api/v1/validate",
            json={
